...
#define LOG_MAGIC 0x4254524653334C47ULL
...
static int drain_shards(cow_tree *t, insert_req **batch, int start_sid)
{
    int n = 0;

    for (int i = 0; i < QUEUE_SHARDS && n < BATCH_MAX; i++) {
        int sid = (start_sid + i) % QUEUE_SHARDS;
        ...
        while (n < BATCH_MAX && s->head) {
            ...
            r->next = NULL;
            batch[n++] = r;
            atomic_fetch_sub_explicit(&t->pending_count, 1, memory_order_relaxed);
        }
...
static void *sync_main(void *arg)
{
    ...
    for (;;) {
        ...
        while (atomic_load_explicit(&t->pending_count, memory_order_acquire) == 0 &&
               !atomic_load_explicit(&t->stop_sync, memory_order_acquire)) {
            pthread_cond_wait(&t->pending_cv, &t->pending_lock);
        }

        if (atomic_load_explicit(&t->pending_count, memory_order_acquire) == 0 &&
            atomic_load_explicit(&t->stop_sync, memory_order_acquire)) {
        ...
        int n = drain_shards(t, batch, rr_sid);
        rr_sid = (rr_sid + 1) % QUEUE_SHARDS;
        if (n <= 0)
            continue;
...
cow_tree *cow_open(const char *path)
{
    ...
    if (pthread_mutex_init(&t->pending_lock, NULL) != 0 ||
        pthread_cond_init(&t->pending_cv, NULL) != 0 ||
    ...
    for (int i = 0; i < QUEUE_SHARDS; i++) {
        if (pthread_mutex_init(&t->shards[i].lock, NULL) != 0) {
            perror("pthread_mutex_init shard");
            exit(EXIT_FAILURE);
        }
    }
...
void cow_close(cow_tree *t)
{
    ...
    pthread_mutex_lock(&t->pending_lock);
    pthread_cond_broadcast(&t->pending_cv);
    pthread_mutex_unlock(&t->pending_lock);
    ...
    pthread_cond_destroy(&t->pending_cv);
...
void cow_insert(cow_tree *t, int64_t key, const char *value)
{
    ...
    atomic_fetch_add_explicit(&t->pending_count, 1, memory_order_relaxed);
    pthread_mutex_lock(&t->pending_lock);
    pthread_cond_signal(&t->pending_cv);
    pthread_mutex_unlock(&t->pending_lock);
