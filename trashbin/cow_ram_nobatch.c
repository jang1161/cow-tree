...
void cow_close(cow_tree *t)
{
...

    {
        ...
        fprintf(stderr,
                "[ram-nobatch] page_cache tl_hit=%lu ram_hit=%lu disk_reads=%lu\n"
                "[ram-nobatch] insert_lock_wait_avg_us=%.2f\n"
                "[ram-nobatch] avg_batch_size=%.2f batches=%lu items=%lu\n"
                "[ram-nobatch] page_appends=%lu\n",
...
void cow_insert(cow_tree *t, int64_t key, const char *value)
{
    ...
    node_id_t root_id = root;
    apply_insert_overlay(&ov, &root_id, key, value);
