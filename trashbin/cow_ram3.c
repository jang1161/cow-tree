...
static inline uint32_t queue_shard_for_thread(void)
{
    uintptr_t tid = (uintptr_t)pthread_self();
    return (uint32_t)(page_hash_u64((uint64_t)tid) & (QUEUE_SHARDS - 1));
}
...
static int pop_one_from_shard(cow_tree *t, uint32_t shard, insert_req **out)
{
    ...
    *out = req;
    atomic_fetch_sub_explicit(&t->queued_items, 1, memory_order_relaxed);
    return 1;
}
...
static int pop_batch(cow_tree *t, insert_req **batch, int max_batch)
{
    ...
    while (!atomic_load_explicit(&t->stop_writer, memory_order_acquire) &&
           atomic_load_explicit(&t->queued_items, memory_order_acquire) == 0)
    {
        usleep(WRITER_BATCH_MIN_WAIT_US);
    }

    if (atomic_load_explicit(&t->stop_writer, memory_order_acquire) &&
        atomic_load_explicit(&t->queued_items, memory_order_acquire) == 0)
    {
        return 0;
    }

    uint32_t start = atomic_fetch_add_explicit(&t->q_rr_hint, 1, memory_order_relaxed) % QUEUE_SHARDS;
    for (uint32_t off = 0; off < QUEUE_SHARDS && n < max_batch; off++)
    {
        uint32_t shard = (start + off) % QUEUE_SHARDS;
        while (n < max_batch)
        {
            insert_req *req = NULL;
            if (!pop_one_from_shard(t, shard, &req))
                break;
            batch[n++] = req;
        }
    }
    ...
    {
        uint64_t qdepth = atomic_load_explicit(&t->queued_items, memory_order_relaxed);
        uint32_t wait_us;
        if (qdepth >= 512)
            wait_us = WRITER_BATCH_WAIT_US + 80;
        else if (qdepth >= 128)
            wait_us = WRITER_BATCH_WAIT_US;
        else if (qdepth >= 32)
            wait_us = WRITER_BATCH_WAIT_US / 2;
        else if (qdepth >= 8)
            wait_us = WRITER_BATCH_MIN_WAIT_US + 20;
        else
            wait_us = WRITER_BATCH_MIN_WAIT_US;

        if (n >= 2 || qdepth >= 8)
        {
            uint32_t remaining = wait_us;
            while (remaining > 0 && n < max_batch)
            {
                uint32_t slice = (remaining > 10) ? 10 : remaining;
                usleep(slice);
                remaining -= slice;

                start = atomic_fetch_add_explicit(&t->q_rr_hint, 1, memory_order_relaxed) % QUEUE_SHARDS;
                int made_progress = 0;
                for (uint32_t off = 0; off < QUEUE_SHARDS && n < max_batch; off++)
                {
                    uint32_t shard = (start + off) % QUEUE_SHARDS;
...
cow_tree *cow_open(const char *path)
{
    ...
    for (uint32_t i = 0; i < QUEUE_SHARDS; i++)
    {
        if (pthread_mutex_init(&t->q_lock[i], NULL) != 0)
        {
            perror("pthread_mutex_init q_lock shard");
    ...
    if (t->fd < 0)
    {
        perror("zbd_open");
        for (uint32_t i = 0; i < QUEUE_SHARDS; i++)
            pthread_mutex_destroy(&t->q_lock[i]);
        pthread_mutex_destroy(&t->flush_lock);
        free(t);
        return NULL;
    }
    ...
    atomic_store_explicit(&t->queued_items, 0, memory_order_release);
    atomic_store_explicit(&t->q_rr_hint, 0, memory_order_release);
...
void cow_close(cow_tree *t)
{
...

    {
        ...
        fprintf(stderr,
                "[ram3] page_cache tl_hit=%lu ram_hit=%lu disk_reads=%lu\n"
                "[ram3] q_lock_wait_avg_us insert=%.2f writer=%.2f\n"
                "[ram3] avg_batch_size=%.2f batches=%lu items=%lu\n"
                "[ram3] page_appends=%lu\n",
...
void cow_insert(cow_tree *t, int64_t key, const char *value)
{
    ...
    uint32_t shard = queue_shard_for_thread();
    ...
    if (t->q_tail[shard])
        ...
        t->q_head[shard] = req;
    t->q_tail[shard] = req;
    atomic_fetch_add_explicit(&t->queued_items, 1, memory_order_release);
    pthread_mutex_unlock(&t->q_lock[shard]);
