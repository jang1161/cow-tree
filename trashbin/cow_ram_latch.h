...
#define WRITER_SHARDS 4
#define LATCH_STRIPES 4096
...
typedef struct
{
    ...
    pthread_t writer_tid[WRITER_SHARDS];
    pthread_mutex_t q_lock[WRITER_SHARDS];
    pthread_cond_t q_cv[WRITER_SHARDS];
    insert_req *q_head[WRITER_SHARDS];
    insert_req *q_tail[WRITER_SHARDS];
    _Atomic(bool) stop_writer;
    pthread_mutex_t commit_lock;
    pthread_rwlock_t latch_stripes[LATCH_STRIPES];
    ...
    _Atomic(uint64_t) stat_latch_wait_ns;
    _Atomic(uint64_t) stat_latch_wait_samples;
