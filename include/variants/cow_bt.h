...
typedef struct
{
    ...
    // bt-style: insert/remove가 tree read lock으로 진입하고, publish 시점만 짧게 직렬화
    ...
    _Atomic(uint64_t) stat_publish_retries;
    _Atomic(uint64_t) stat_cow_shared;
    _Atomic(uint64_t) stat_cow_unique;
