...
void cow_close(cow_tree *t)
{
...

    {
        ...
        fprintf(stderr,
                "[ram] page_cache tl_hit=%lu ram_hit=%lu disk_reads=%lu\n"
                "[ram] q_lock_wait_avg_us insert=%.2f writer=%.2f\n"
                "[ram] avg_batch_size=%.2f batches=%lu items=%lu\n"
            "[ram] queue_depth avg=%.2f max=%lu writer_empty_waits=%lu\n"
            "[ram] overlay_nodes avg=%.2f max=%lu\n"
            "[ram] append_retries=%lu zone_rotations=%lu\n"
            "[ram] sampled_latency_us append=%.2f batch=%.2f\n"
                "[ram] sampled_stage_us sort=%.2f apply=%.2f flush=%.2f\n"
                "[ram] page_appends=%lu\n",
