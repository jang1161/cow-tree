...
#define PROF_SAMPLE_MASK 127U
#define FLUSH_ASYNC_MAX_WORKERS 8
#define LATENCY_SAMPLE_CAP 65536
...
static void latency_sample_push(uint64_t **buf, size_t *used, size_t *cap, uint64_t v)
{
    if (*used == LATENCY_SAMPLE_CAP)
        return;

    if (*used == *cap)
    {
        size_t new_cap = (*cap == 0) ? 1024 : (*cap << 1);
        if (new_cap > LATENCY_SAMPLE_CAP)
            new_cap = LATENCY_SAMPLE_CAP;

        uint64_t *next = realloc(*buf, new_cap * sizeof(*next));
        if (!next)
        {
            perror("realloc latency sample");
            exit(EXIT_FAILURE);
        }
        *buf = next;
        *cap = new_cap;
...
static double latency_percentile_us(const uint64_t *samples, size_t n, double pct)
{
    if (n == 0)
        return 0.0;
    ...
    if (!tmp)
    {
        perror("malloc latency percentile tmp");
        exit(EXIT_FAILURE);
    }
    memcpy(tmp, samples, n * sizeof(*tmp));
    qsort(tmp, n, sizeof(*tmp), cmp_u64);

    double pos = ((double)(n - 1) * pct) / 100.0;
    size_t idx = (size_t)(pos + 0.5);
    if (idx >= n)
        idx = n - 1;

    double out = (double)tmp[idx] / 1000.0;
    free(tmp);
    return out;
}
...
{
    pthread_mutex_lock(&t->ram_table_lock);
    if (!t->ram_slots)
    {
        pthread_mutex_unlock(&t->ram_table_lock);
        return;
    }
...
static void overlay_mark_depth(overlay_state *ov, node_id_t id,
                               uint32_t depth, uint8_t *seen,
                               uint32_t *depths, uint32_t *max_depth)
{
    if (id == INVALID_PGN)
        return;
    ...
    for (uint32_t i = 0; i < n->node.num_keys; i++)
    {
        overlay_mark_depth(ov, n->node.internal[i].child, depth + 1, seen, depths, max_depth);
    }
    overlay_mark_depth(ov, n->node.pointer, depth + 1, seen, depths, max_depth);
}

static void flush_one_overlay_idx(overlay_state *ov, int idx)
{
    overlay_node *n = &ov->arr[idx];
    if (n->flushed)
        return;
    ...
    if (!out.is_leaf)
    {
        for (uint32_t i = 0; i < out.num_keys; i++)
        {
            ...
            int cidx = overlay_find_idx(ov, (node_id_t)old_child);
            if (cidx >= 0)
            {
                overlay_node *cn = &ov->arr[cidx];
                if (!cn->flushed)
                {
                    fprintf(stderr, "overlay async flush order violation %lu\n", (unsigned long)old_child);
                    exit(EXIT_FAILURE);
                }
                pagenum_t new_child = cn->flushed_pn;
        ...
        int pidx = overlay_find_idx(ov, (node_id_t)old_ptr);
        if (pidx >= 0)
        {
            overlay_node *pn = &ov->arr[pidx];
            if (!pn->flushed)
            {
                fprintf(stderr, "overlay async flush order violation %lu\n", (unsigned long)old_ptr);
                exit(EXIT_FAILURE);
            }
            pagenum_t new_ptr = pn->flushed_pn;
            if (old_ptr != new_ptr)
                changed = 1;
            out.pointer = new_ptr;
        }
...
static void *flush_level_worker(void *arg)
{
    flush_level_ctx *ctx = (flush_level_ctx *)arg;
    for (;;)
    {
        size_t i = atomic_fetch_add_explicit(&ctx->next, 1, memory_order_relaxed);
        if (i >= ctx->count)
            break;
        flush_one_overlay_idx(ctx->ov, ctx->indices[i]);
    }
    return NULL;
}

static pagenum_t flush_overlay_async(overlay_state *ov, node_id_t root_id, int do_sample)
{
    if (root_id == INVALID_PGN)
        return INVALID_PGN;
    ...
    if (root_idx < 0)
        return (pagenum_t)root_id;

    uint8_t *seen = calloc(ov->len, sizeof(*seen));
    uint32_t *depths = calloc(ov->len, sizeof(*depths));
    int *level_nodes = malloc(ov->len * sizeof(*level_nodes));
    if (!seen || !depths || !level_nodes)
    {
        free(seen);
        free(depths);
        free(level_nodes);
        perror("calloc async flush metadata");
        exit(EXIT_FAILURE);
    }
    ...
    {
        ov->arr[i].flushed = 0;
        ov->arr[i].flushed_pn = INVALID_PGN;
    }

    uint32_t max_depth = 0;
    overlay_mark_depth(ov, root_id, 0, seen, depths, &max_depth);

    uint64_t levels = 0;
    uint64_t workers_used_sum = 0;
    uint64_t level_width_sum = 0;
    uint64_t level_width_max = 0;
    uint64_t flush_t0 = do_sample ? monotonic_ns() : 0;

    for (int d = (int)max_depth; d >= 0; d--)
    {
        size_t count = 0;
        for (size_t i = 0; i < ov->len; i++)
        {
            if (seen[i] && depths[i] == (uint32_t)d)
                level_nodes[count++] = (int)i;
        }
        ...
        levels++;
        level_width_sum += (uint64_t)count;
        if ((uint64_t)count > level_width_max)
            level_width_max = (uint64_t)count;

        int workers = (count < FLUSH_ASYNC_MAX_WORKERS) ? (int)count : FLUSH_ASYNC_MAX_WORKERS;
        workers_used_sum += (uint64_t)workers;

        if (workers <= 1)
        {
            for (size_t i = 0; i < count; i++)
                flush_one_overlay_idx(ov, level_nodes[i]);
            continue;
        }

        flush_level_ctx ctx;
        ctx.ov = ov;
        ctx.indices = level_nodes;
        ctx.count = count;
        atomic_init(&ctx.next, 0);

        pthread_t tids[FLUSH_ASYNC_MAX_WORKERS];
        for (int i = 0; i < workers; i++)
        {
            if (pthread_create(&tids[i], NULL, flush_level_worker, &ctx) != 0)
            {
                perror("pthread_create async flush worker");
                exit(EXIT_FAILURE);
            }
        }
        for (int i = 0; i < workers; i++)
    ...
    if (do_sample)
    {
        uint64_t dt = monotonic_ns() - flush_t0;
        atomic_fetch_add_explicit(&ov->t->stat_async_flush_parallel_ns_sum, dt, memory_order_relaxed);
        atomic_fetch_add_explicit(&ov->t->stat_async_flush_parallel_ns_samples, 1, memory_order_relaxed);
    }
    atomic_fetch_add_explicit(&ov->t->stat_async_flush_levels_sum, levels, memory_order_relaxed);
    atomic_fetch_add_explicit(&ov->t->stat_async_flush_levels_samples, 1, memory_order_relaxed);
    atomic_fetch_add_explicit(&ov->t->stat_async_flush_workers_sum, workers_used_sum, memory_order_relaxed);
    atomic_fetch_add_explicit(&ov->t->stat_async_flush_workers_samples, 1, memory_order_relaxed);
    atomic_fetch_add_explicit(&ov->t->stat_async_flush_level_width_sum, level_width_sum, memory_order_relaxed);
    atomic_fetch_add_explicit(&ov->t->stat_async_flush_level_width_samples, levels, memory_order_relaxed);
    stat_update_max_u64(&ov->t->stat_async_flush_level_width_max, level_width_max);

    pagenum_t new_root = ov->arr[root_idx].flushed_pn;
    free(seen);
    free(depths);
    free(level_nodes);
    return new_root;
...
static void *writer_main(void *arg)
{
    ...
    for (;;)
    {
        ...
        pagenum_t new_root = (root_id == INVALID_PGN) ? INVALID_PGN : flush_overlay_async(&ov, root_id, do_sample);
        if (do_sample)
        {
            uint64_t dt = monotonic_ns() - stage_t0;
            ...
            latency_sample_push(&t->stat_flush_latency_samples_ns,
                                &t->stat_flush_latency_samples_used,
                                &t->stat_flush_latency_samples_cap,
                                dt);
        }
        ...
        if (do_sample)
        {
            ...
            latency_sample_push(&t->stat_batch_latency_samples_ns,
                                &t->stat_batch_latency_samples_used,
                                &t->stat_batch_latency_samples_cap,
                                dt);
        }
...
cow_tree *cow_open(const char *path)
{
    ...
    if (pthread_mutex_init(&t->ram_table_lock, NULL) != 0)
    {
        perror("pthread_mutex_init ram_table_lock");
        free(t);
        return NULL;
    }
    ...
    if (pthread_mutex_init(&t->flush_lock, NULL) != 0)
    {
        ...
        pthread_mutex_destroy(&t->ram_table_lock);
        free(t);
        return NULL;
    }
...
void cow_close(cow_tree *t)
{
...

    {
        ...
        uint64_t async_levels_sum = atomic_load_explicit(&t->stat_async_flush_levels_sum, memory_order_relaxed);
        uint64_t async_levels_samples = atomic_load_explicit(&t->stat_async_flush_levels_samples, memory_order_relaxed);
        uint64_t async_workers_sum = atomic_load_explicit(&t->stat_async_flush_workers_sum, memory_order_relaxed);
        uint64_t async_workers_samples = atomic_load_explicit(&t->stat_async_flush_workers_samples, memory_order_relaxed);
        uint64_t async_width_sum = atomic_load_explicit(&t->stat_async_flush_level_width_sum, memory_order_relaxed);
        uint64_t async_width_samples = atomic_load_explicit(&t->stat_async_flush_level_width_samples, memory_order_relaxed);
        uint64_t async_width_max = atomic_load_explicit(&t->stat_async_flush_level_width_max, memory_order_relaxed);
        uint64_t async_parallel_ns_sum = atomic_load_explicit(&t->stat_async_flush_parallel_ns_sum, memory_order_relaxed);
        uint64_t async_parallel_ns_samples = atomic_load_explicit(&t->stat_async_flush_parallel_ns_samples, memory_order_relaxed);
        ...
        double async_levels_avg = (async_levels_samples > 0) ? ((double)async_levels_sum / (double)async_levels_samples) : 0.0;
        double async_workers_avg = (async_workers_samples > 0) ? ((double)async_workers_sum / (double)async_workers_samples) : 0.0;
        double async_width_avg = (async_width_samples > 0) ? ((double)async_width_sum / (double)async_width_samples) : 0.0;
        double async_parallel_avg_us = (async_parallel_ns_samples > 0) ? ((double)async_parallel_ns_sum / (double)async_parallel_ns_samples / 1000.0) : 0.0;
        double batch_p50_us = latency_percentile_us(t->stat_batch_latency_samples_ns, t->stat_batch_latency_samples_used, 50.0);
        double batch_p95_us = latency_percentile_us(t->stat_batch_latency_samples_ns, t->stat_batch_latency_samples_used, 95.0);
        double batch_p99_us = latency_percentile_us(t->stat_batch_latency_samples_ns, t->stat_batch_latency_samples_used, 99.0);
        double flush_p50_us = latency_percentile_us(t->stat_flush_latency_samples_ns, t->stat_flush_latency_samples_used, 50.0);
        double flush_p95_us = latency_percentile_us(t->stat_flush_latency_samples_ns, t->stat_flush_latency_samples_used, 95.0);
        double flush_p99_us = latency_percentile_us(t->stat_flush_latency_samples_ns, t->stat_flush_latency_samples_used, 99.0);

        fprintf(stderr,
            "[ram-async] q_lock_wait_avg_us insert=%.2f writer=%.2f\n"
            "[ram-async] avg_batch_size=%.2f batches=%lu items=%lu\n"
            "[ram-async] queue_depth avg=%.2f max=%lu writer_empty_waits=%lu\n"
            "[ram-async] overlay_nodes avg=%.2f max=%lu\n"
            "[ram-async] sampled_latency_us append=%.2f batch=%.2f\n"
            "[ram-async] sampled_stage_us sort=%.2f apply=%.2f flush=%.2f\n"
            "[ram-async] sampled_percentile_us batch_p50=%.2f batch_p95=%.2f batch_p99=%.2f flush_p50=%.2f flush_p95=%.2f flush_p99=%.2f n_batch=%lu n_flush=%lu\n"
            "[ram-async] sampled_flush_async_us parallel=%.2f levels=%.2f workers=%.2f level_width=%.2f max_width=%lu\n"
            "[ram-async] page_appends=%lu\n",
                qwait_ins_avg_us,
                qwait_w_avg_us,
                avg_batch_size,
            ...
            batch_lat_avg_us,
                ...
                flush_p95_us,
                flush_p99_us,
                (unsigned long)t->stat_batch_latency_samples_used,
                (unsigned long)t->stat_flush_latency_samples_used,
                async_parallel_avg_us,
                async_levels_avg,
                async_workers_avg,
                async_width_avg,
                (unsigned long)async_width_max,
    ...
    if (t->direct_fd >= 0)
        close(t->direct_fd);
    free(t->stat_batch_latency_samples_ns);
    free(t->stat_flush_latency_samples_ns);
    ram_table_destroy(t);
    free(t->zones);
    free(t->zone_wp_bytes);
    free(t->zone_full);
