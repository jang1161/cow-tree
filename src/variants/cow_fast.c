/*
 * cow_fast.c — COW B-tree variant with batched NVMe zone append.
 *
 * Key difference from cow_ram_stage2:
 *   flush_overlay_batch() uses a WP pre-reservation trick:
 *     1. Read the zone write-pointer (WP) before traversal → predict page numbers
 *     2. Post-order DFS: resolve child pns bottom-up, collect dirty pages into buffer
 *     3. Issue ONE nvme_zns_append for all dirty pages (instead of one per page)
 *     4. Update ram_table
 *
 *   This eliminates the per-page NVMe round-trip latency overhead.
 *   ZASL (zone append size limit) is handled by splitting into ≤MAX_APPEND_PAGES chunks.
 */
...
#define TXG_BATCH_MAX        512
#define TXG_BATCH_WAIT_US    200
#define TXG_BATCH_MIN_WAIT_US 50
#define TXG_COMMIT_MERGE_JOBS   4
#define TXG_COMMIT_MERGE_ITEMS 1024

/* Max pages per single nvme_zns_append command (ZASL-safe) */
...
typedef struct
{
    page *buf;           /* contiguous page write buffer */
    int n;               /* pages collected so far */
    int cap;             /* buf capacity in pages */
    pagenum_t start_pn;  /* page number of buf[0] */
    int zone_pages_left; /* pages remaining in current zone before overflow */
...
static void ram_table_init(cow_tree *t)
{
    ...
    if (!t->ram_slots) { perror("calloc ram table"); exit(EXIT_FAILURE); }
}

static void ram_table_destroy(cow_tree *t)
{
    if (!t->ram_slots) return;
    ram_page_slot *slots = ram_slots(t);
    for (size_t i = 0; i < t->ram_cap; i++)
        if (slots[i].used) free(slots[i].data);
    free(t->ram_slots);
    t->ram_slots = NULL;
    t->ram_cap = 0;
    t->ram_used = 0;
...
static void ram_table_grow(cow_tree *t)
{
    ...
    if (!new_slots) { perror("calloc ram table grow"); exit(EXIT_FAILURE); }

    for (size_t i = 0; i < old_cap; i++)
    {
        if (!old_slots[i].used) continue;
        size_t pos = (size_t)(page_hash_u64((uint64_t)old_slots[i].pn) & (new_cap - 1));
        while (new_slots[pos].used) pos = (pos + 1) & (new_cap - 1);
...
static int ram_table_lookup(cow_tree *t, pagenum_t pn, page *dst)
{
    ...
    for (;;)
    {
        ...
        if (!s->used) return 0;
        if (s->pn == pn) { *dst = *s->data; return 1; }
...
{
    if (!t->ram_slots) ram_table_init(t);
    if ((t->ram_used + 1) * 10 >= t->ram_cap * 7) ram_table_grow(t);
    ...
    for (;;)
    {
        ...
        if (!s->used)
        {
            ...
            if (!s->data) { perror("malloc ram page"); exit(EXIT_FAILURE); }
            *s->data = *src;
            s->pn = pn;
            s->used = 1;
            t->ram_used++;
...
static void free_tls_buf(void *ptr) { free(ptr); }
...
static void *get_tls_direct_read_buf(void)
{
    ...
    { perror("pthread_setspecific"); free(buf); exit(EXIT_FAILURE); }
    return buf;
}
...
static insert_req *get_tls_insert_req(void)
{
    ...
    { perror("pthread_once req_tls_key"); exit(EXIT_FAILURE); }
    insert_req *req = pthread_getspecific(req_tls_key);
    if (req) return req;

    req = calloc(1, sizeof(*req));
    if (!req) { perror("calloc tls insert_req"); exit(EXIT_FAILURE); }
    if (pthread_mutex_init(&req->done_lock, NULL) != 0)
    { perror("pthread_mutex_init tls req.done_lock"); free(req); exit(EXIT_FAILURE); }
    if (pthread_cond_init(&req->done_cv, NULL) != 0)
    {
        perror("pthread_cond_init tls req.done_cv");
        pthread_mutex_destroy(&req->done_lock); free(req); exit(EXIT_FAILURE);
    }
...
static int is_empty_snapshot(pagenum_t root_pn) { return root_pn == INVALID_PGN; }
...
static void read_tree_snapshot(cow_tree *t, pagenum_t *root_pn, uint64_t *seq_no)
{
    for (;;)
    {
        ...
        if (s1 == s2 && ((s2 & 1ULL) == 0)) { *root_pn = r; *seq_no = s2; return; }
    }
}
...
static void load_page(cow_tree *t, pagenum_t pn, page *dst)
{
    ...
    if (ram_table_lookup(t, pn, dst))
    {
        ...
        slot->valid = 1; slot->pn = pn; slot->data = *dst;
        return;
    }
    ...
    if (t->direct_fd >= 0)
    {
        ...
        if (n != PAGE_SIZE) { perror("load_page(O_DIRECT)"); exit(EXIT_FAILURE); }
        memcpy(dst, raw, PAGE_SIZE);
    }
    else
    {
        if (pread(t->fd, dst, PAGE_SIZE, off) != PAGE_SIZE)
        { perror("load_page"); exit(EXIT_FAILURE); }
    }
...
/* ------------------------------------------------------------------ */
/* NVMe ZNS append helpers                                             */
/* ------------------------------------------------------------------ */

/* Append a single contiguous buffer of n_pages to zone_id.
 * Returns 0 on success; sets *out_start_pn to the first written page number
 * and updates zone_wp_bytes.
 */
static int zone_append_raw_n(cow_tree *t, uint32_t zone_id,
                               const void *buf, int n_pages,
                               pagenum_t *out_start_pn,
                               uint64_t *out_new_wp_bytes)
{
    __u64 zslba = t->zones[zone_id].start / t->info.lblock_size;
    uint32_t total_lb = (uint32_t)n_pages * (uint32_t)(PAGE_SIZE / t->info.lblock_size);
    ...
    uint64_t new_wp = (result + (uint64_t)nlb + 1) * t->info.lblock_size;
    if (out_new_wp_bytes) *out_new_wp_bytes = new_wp;
    if (out_start_pn)
        *out_start_pn = (pagenum_t)(result * t->info.lblock_size / PAGE_SIZE);

    /* Update our WP tracking */
    uint64_t cur = atomic_load_explicit(&t->zone_wp_bytes[zone_id], memory_order_acquire);
    while (new_wp > cur &&
           !atomic_compare_exchange_weak_explicit(
               &t->zone_wp_bytes[zone_id], &cur, new_wp,
               memory_order_acq_rel, memory_order_acquire))
    { }
...
/* Append exactly one page (used for meta zone writes and single-page fallback) */
static int zone_append_raw_nolock(cow_tree *t, uint32_t zone_id, const void *buf,
                                   pagenum_t *out_pn, uint64_t *out_wp_bytes)
{
    pagenum_t start;
    uint64_t new_wp;
    if (zone_append_raw_n(t, zone_id, buf, 1, &start, &new_wp) != 0)
        return -1;
    if (out_pn) *out_pn = start;
    if (out_wp_bytes) *out_wp_bytes = new_wp;
    return 0;
}

/* Append a single page to the current data zone (handles zone switching). */
static pagenum_t cow_append_page(cow_tree *t, page *p)
{
    ...
    for (;;)
    {
        ...
        uint64_t old_wp = atomic_fetch_add_explicit(
            &t->zone_wp_bytes[zone_id], PAGE_SIZE, memory_order_acq_rel);
        uint64_t zone_end = t->zones[zone_id].start + t->zones[zone_id].capacity;

        if (old_wp + PAGE_SIZE > zone_end)
        {
            atomic_fetch_sub_explicit(&t->zone_wp_bytes[zone_id], PAGE_SIZE, memory_order_acq_rel);
            ...
            atomic_compare_exchange_weak_explicit(
                &t->current_zone, &expected, zone_next(t, zone_id),
                memory_order_acq_rel, memory_order_acquire);
            continue;
        }
...
static void activate_meta_zone(cow_tree *t, uint32_t zone_id, uint64_t version)
{
    ...
    { perror("nvme_zns_append(meta_zone_header)"); exit(EXIT_FAILURE); }
...
static void load_superblock(cow_tree *t)
{
    ...
    else if (wp0 > 0 && wp1 > 0)
    {
        ...
        { t->durable_sb = sb0; t->active_zone = META_ZONE_0; t->meta_wp = wp0 + 1; t->version = zh0.version; }
        else
        { t->durable_sb = sb1; t->active_zone = META_ZONE_1; t->meta_wp = wp1 + 1; t->version = zh1.version; }
    }
    else if (wp0 > 0)
    { t->durable_sb = sb0; t->active_zone = META_ZONE_0; t->meta_wp = wp0 + 1; t->version = zh0.version; }
    else
...
static void *sb_flusher_thread(void *arg)
{
    ...
    while (!atomic_load_explicit(&t->flusher_stop, memory_order_acquire))
    {
        ...
        if (!atomic_exchange_explicit(&t->dirty, false, memory_order_acq_rel)) continue;
        pagenum_t root; uint64_t seq;
...
static void overlay_index_grow(overlay_state *ov)
{
    ...
    if (!new_tab) { perror("malloc overlay idx_table"); exit(EXIT_FAILURE); }
    for (size_t i = 0; i < new_cap; i++) new_tab[i] = (size_t)-1;
    for (size_t i = 0; i < ov->len; i++)
    {
        node_id_t id = ov->arr[i].id;
        size_t m = new_cap - 1;
        size_t pos = (size_t)(overlay_hash_u64((uint64_t)id) & m);
        while (new_tab[pos] != (size_t)-1) pos = (pos + 1) & m;
        new_tab[pos] = i;
    }
    free(ov->idx_table);
...
static void overlay_index_insert(overlay_state *ov, node_id_t id, size_t idx)
{
    ...
    size_t m = ov->idx_cap - 1;
    size_t pos = (size_t)(overlay_hash_u64((uint64_t)id) & m);
    while (ov->idx_table[pos] != (size_t)-1) pos = (pos + 1) & m;
...
static overlay_node *overlay_add_node(overlay_state *ov, node_id_t id, const page *src)
{
    if (ov->len == ov->cap)
    {
        ...
        if (!next) { perror("realloc overlay"); exit(EXIT_FAILURE); }
        ov->arr = next;
        ov->cap = new_cap;
    }
    size_t idx = ov->len;
...
static overlay_node *overlay_get_mut(overlay_state *ov, node_id_t id)
{
    ...
    if (idx >= 0) return &ov->arr[idx];
    if (is_temp_id(id)) return NULL;
    page p;
...
/* ------------------------------------------------------------------ */
/* B-tree apply (single-threaded, same as ram_stage2)                 */
/* ------------------------------------------------------------------ */
...
static void apply_insert_overlay(overlay_state *ov, node_id_t *root_id, int64_t key, const char *value)
{
    ...
    while (1)
    {
        ...
        if (!n) { fprintf(stderr, "overlay: missing node %lu\n", (unsigned long)cur); exit(EXIT_FAILURE); }
        if (n->node.is_leaf) break;
        if (path.depth >= MAX_HEIGHT) { fprintf(stderr, "tree depth exceeded MAX_HEIGHT\n"); exit(EXIT_FAILURE); }

        uint32_t idx = RIGHTMOST_IDX;
        for (uint32_t i = 0; i < n->node.num_keys; i++)
        {
            if (key < (int64_t)n->node.internal[i].key) { idx = i; break; }
        }
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < LEAF_ORDER - sp; i++) right_n->node.leaf[i] = tmp[sp + i];
    ...
    while (path.depth > 0)
    {
        ...
        if (par->num_keys < INTERNAL_ORDER - 1)
        {
            ...
            if (pos == par->num_keys) par->pointer = carry_right;
        ...
        for (uint32_t j = 0; j < pos; j++) tkeys[j] = (int64_t)par->internal[j].key;
        tkeys[pos] = carry_key;
        for (uint32_t j = pos; j < INTERNAL_ORDER - 1; j++) tkeys[j + 1] = (int64_t)par->internal[j].key;
        for (uint32_t j = 0; j < pos; j++) tchld[j] = par->internal[j].child;
...
/* ------------------------------------------------------------------ */
/* Batched flush: pre-reserve WP, single nvme_zns_append per chunk    */
/* ------------------------------------------------------------------ */

/*
 * Post-order DFS: collect dirty/changed pages into buf[] in a single pass.
 * Page numbers are pre-computed from start_pn+counter (WP pre-reservation).
 * Children are resolved before parents since we traverse bottom-up.
 */
static pagenum_t flush_collect_node(flush_batch_ctx *ctx, overlay_state *ov, node_id_t id)
{
    if (id == INVALID_PGN) return INVALID_PGN;

    if (!is_temp_id(id))
    {
        int idx = overlay_find_idx(ov, id);
        if (idx < 0) return (pagenum_t)id; /* not in overlay, use real pn as-is */

        overlay_node *n = &ov->arr[idx];
        if (n->flushed) return n->flushed_pn;
        ...
        if (!out.is_leaf)
        {
            ...
            {
                pagenum_t nc = flush_collect_node(ctx, ov, (node_id_t)out.internal[i].child);
                if (nc != out.internal[i].child) { changed = 1; out.internal[i].child = nc; }
            }
            pagenum_t np = flush_collect_node(ctx, ov, (node_id_t)out.pointer);
            if (np != out.pointer) { changed = 1; out.pointer = np; }
        }
        ...
        /* Dirty or pointer-changed: must re-write */
        if (ctx->n >= ctx->cap)
        {
            ctx->cap = ctx->cap ? ctx->cap * 2 : 256;
            ctx->buf = realloc(ctx->buf, (size_t)ctx->cap * PAGE_SIZE);
            if (!ctx->buf) { perror("realloc flush buf"); exit(EXIT_FAILURE); }
        }
        pagenum_t pn = ctx->start_pn + (pagenum_t)ctx->n;
        out.pn = pn;
        ctx->buf[ctx->n] = out;
        ctx->n++;
        n->flushed_pn = pn;
    ...
    /* Temp ID: always dirty */
    int idx = overlay_find_idx(ov, id);
    if (idx < 0)
    {
        fprintf(stderr, "flush_collect: temp node not found %lu\n", (unsigned long)id);
        exit(EXIT_FAILURE);
    }
    overlay_node *n = &ov->arr[idx];
    if (n->flushed) return n->flushed_pn;
    ...
    if (ctx->n >= ctx->cap)
    {
        ctx->cap = ctx->cap ? ctx->cap * 2 : 256;
        ctx->buf = realloc(ctx->buf, (size_t)ctx->cap * PAGE_SIZE);
        if (!ctx->buf) { perror("realloc flush buf temp"); exit(EXIT_FAILURE); }
    }
    pagenum_t pn = ctx->start_pn + (pagenum_t)ctx->n;
    out.pn = pn;
    ctx->buf[ctx->n] = out;
...
/*
 * Flush all dirty pages for root_id using batched NVMe append.
 * Returns the new root page number.
 *
 * Single-committer invariant: we are the only writer to data zones,
 * so reading zone_wp_bytes gives the exact next-write offset.
 */
static pagenum_t flush_overlay_batch(overlay_state *ov, cow_tree *t, node_id_t root_id)
{
    if (root_id == INVALID_PGN) return INVALID_PGN;

    /* ---- determine the zone and its remaining capacity ---- */
    uint32_t zone_id = atomic_load_explicit(&t->current_zone, memory_order_acquire);
    if (zone_id < DATA_ZONE_START)
        zone_id = DATA_ZONE_START;
    /* skip full zones */
    while (zone_id < t->info.nr_zones &&
           atomic_load_explicit(&t->zone_full[zone_id], memory_order_acquire))
        zone_id++;
    if (zone_id >= t->info.nr_zones)
    {
        fprintf(stderr, "flush_overlay_batch: zones exhausted\n");
        exit(EXIT_FAILURE);
    }

    uint64_t zone_end    = t->zones[zone_id].start + t->zones[zone_id].capacity;
    uint64_t cur_wp      = atomic_load_explicit(&t->zone_wp_bytes[zone_id], memory_order_acquire);
    uint64_t zone_remain = (zone_end > cur_wp) ? (zone_end - cur_wp) : 0;
    int      pages_fit   = (int)(zone_remain / PAGE_SIZE);

    /* ---- estimate dirty count so we can zone-switch before collection ----
     * We count all overlay entries that are dirty or temp.
     * This is an upper bound; actual dirty set may be smaller after sharing.
     */
    int est_dirty = 0;
    for (size_t i = 0; i < ov->len; i++)
    {
        overlay_node *n = &ov->arr[i];
        if (n->dirty || is_temp_id(n->id)) est_dirty++;
    }

    /* If the zone can't hold the estimated pages, switch now */
    if (est_dirty > pages_fit)
    {
        atomic_store_explicit(&t->zone_full[zone_id], 1, memory_order_release);
        atomic_fetch_add_explicit(&t->stat_zone_rotations, 1, memory_order_relaxed);
        uint32_t next = zone_id + 1;
        atomic_compare_exchange_weak_explicit(
            &t->current_zone, &zone_id, next,
            memory_order_acq_rel, memory_order_acquire);
        zone_id = next;
        while (zone_id < t->info.nr_zones &&
               atomic_load_explicit(&t->zone_full[zone_id], memory_order_acquire))
            zone_id++;
        if (zone_id >= t->info.nr_zones)
        { fprintf(stderr, "flush_overlay_batch: zones exhausted after switch\n"); exit(EXIT_FAILURE); }
        cur_wp    = atomic_load_explicit(&t->zone_wp_bytes[zone_id], memory_order_acquire);
        zone_end  = t->zones[zone_id].start + t->zones[zone_id].capacity;
        zone_remain = (zone_end > cur_wp) ? (zone_end - cur_wp) : 0;
        pages_fit = (int)(zone_remain / PAGE_SIZE);
    }

    /* ---- pre-reserve: start_pn = current WP / PAGE_SIZE ---- */
    pagenum_t start_pn = (pagenum_t)(cur_wp / PAGE_SIZE);

    /* ---- collect pages in post-order DFS ---- */
    flush_batch_ctx ctx;
    ctx.t            = t;
    ctx.buf          = NULL;
    ctx.n            = 0;
    ctx.cap          = 0;
    ctx.start_pn     = start_pn;
    ctx.zone_pages_left = pages_fit;
    ctx.zone_id      = zone_id;

    pagenum_t new_root = flush_collect_node(&ctx, ov, root_id);

    if (ctx.n == 0)
    {
        free(ctx.buf);
        return new_root; /* nothing to write */
    }

    /* ---- verify fit (should not fail given est_dirty check above) ---- */
    if (ctx.n > ctx.zone_pages_left)
    {
        /* Rare: estimation was too low (path sharing reduced actual dirty count, but
         * zone ran out mid-collection for a different reason). Fall back page by page. */
        fprintf(stderr, "[cow_fast] warning: zone overflow fallback (%d > %d)\n",
                ctx.n, ctx.zone_pages_left);
    ...
    /* ---- batched NVMe append in MAX_APPEND_PAGES chunks ---- */
    int pages_written = 0;
    uint64_t append_t0 = monotonic_ns();
    while (pages_written < ctx.n)
    {
        int chunk = ctx.n - pages_written;
        if (chunk > MAX_APPEND_PAGES) chunk = MAX_APPEND_PAGES;

        pagenum_t actual_start;
        uint64_t new_wp;
        if (zone_append_raw_n(t, zone_id,
                               &ctx.buf[pages_written], chunk,
                               &actual_start, &new_wp) != 0)
        {
            /* Fallback to single-page on error */
            for (int i = pages_written; i < ctx.n; i++)
                cow_append_page(t, &ctx.buf[i]);
            break;
        }

        /* Sanity: actual start should match our prediction */
        pagenum_t expected = start_pn + (pagenum_t)pages_written;
        if (actual_start != expected)
        {
            fprintf(stderr, "[cow_fast] WP mismatch: expected pn=%lu got %lu\n",
                    (unsigned long)expected, (unsigned long)actual_start);
            /* Adjust pns in ram_table using actual positions */
            for (int i = 0; i < chunk; i++)
                ctx.buf[pages_written + i].pn = actual_start + (pagenum_t)i;
        }

        /* Update ram_table */
        for (int i = 0; i < chunk; i++)
        {
            pagenum_t pn = actual_start + (pagenum_t)i;
            ctx.buf[pages_written + i].pn = pn;
            ram_table_insert(t, pn, &ctx.buf[pages_written + i]);
        }

        atomic_fetch_add_explicit(&t->stat_multi_append_commands, 1, memory_order_relaxed);
        atomic_fetch_add_explicit(&t->stat_multi_append_pages, (uint64_t)chunk, memory_order_relaxed);
        atomic_fetch_add_explicit(&t->stat_page_appends, (uint64_t)chunk, memory_order_relaxed);
    ...
    uint64_t append_dt = monotonic_ns() - append_t0;
    atomic_fetch_add_explicit(&t->stat_append_latency_ns_sum, append_dt, memory_order_relaxed);
    atomic_fetch_add_explicit(&t->stat_append_latency_ns_samples, 1, memory_order_relaxed);
...
/* ------------------------------------------------------------------ */
/* Batch collection and processing pipeline                            */
/* ------------------------------------------------------------------ */

static void stage2_enqueue_job(cow_tree *t, txg_batch_job *job)
{
    pthread_mutex_lock(&t->stage2_lock);
    if (t->stage2_tail) t->stage2_tail->next = job;
    else t->stage2_head = job;
    t->stage2_tail = job;
...
static txg_batch_job *stage2_dequeue_job(cow_tree *t)
{
    ...
    if (job)
    {
        ...
        if (t->stage2_head == NULL) t->stage2_tail = NULL;
    }
    pthread_mutex_unlock(&t->stage2_lock);
    return job;
}
...
static int pop_batch(cow_tree *t, insert_req **batch, int max_batch)
{
    ...
    if (n < max_batch && !atomic_load_explicit(&t->stop_sync, memory_order_acquire))
    {
        ...
        if (n >= 4 || qdepth_cur >= 8)
        {
            uint32_t wait_us = (n >= 32 || qdepth_cur >= 64) ? TXG_BATCH_WAIT_US : TXG_BATCH_MIN_WAIT_US;
            struct timespec now;
            ...
            for (;;)
            {
                if (n >= max_batch || atomic_load_explicit(&t->stop_sync, memory_order_acquire)) break;
...
static int cmp_batch_item(const void *a, const void *b)
{
    ...
    if (x->req->key < y->req->key) return -1;
    if (x->req->key > y->req->key) return 1;
...
static void process_txg_jobs(cow_tree *t, txg_batch_job **jobs, int njobs, int free_jobs)
{
    ...
    for (int j = 0; j < njobs; j++)
    {
        ...
        if (cur->start_ns < start_ns) start_ns = cur->start_ns;
        for (int i = 0; i < cur->n; i++)
        {
            if (merged_n >= TXG_COMMIT_MERGE_ITEMS) break;
    ...
    if (merged_n <= 0)
    {
        if (free_jobs) for (int j = 0; j < njobs; j++) free(jobs[j]);
        return;
    }
    ...
    pagenum_t new_root = (root_id == INVALID_PGN)
        ? INVALID_PGN
        : flush_overlay_batch(&ov, t, root_id);
    ...
    atomic_fetch_add_explicit(&t->stat_batch_items, (uint64_t)merged_n, memory_order_relaxed);
}
...
static void *txg_batch_main(void *arg)
{
    ...
    for (;;)
    {
        ...
        /* Check if stage2 is empty and we can do inline fast path */
        int inline_fast = 0;
        if (n <= 2)
        {
            uint64_t qdepth_cur = atomic_load_explicit(&t->stat_queue_depth_current, memory_order_relaxed);
        ...
        if (!inline_fast)
        {
            ...
            if (!job) { perror("malloc txg_batch_job"); exit(EXIT_FAILURE); }
        }
...
static void *txg_commit_main(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (!job)
        {
            if (atomic_load_explicit(&t->stop_commit, memory_order_acquire)) break;
            continue;
        }
...
#define INIT_MUTEX(m) do { \
    if (pthread_mutex_init(&t->m, NULL) != 0) { \
        perror("pthread_mutex_init " #m); goto fail_early; \
    } } while (0)
#define INIT_COND(c) do { \
    if (pthread_cond_init(&t->c, NULL) != 0) { \
        perror("pthread_cond_init " #c); goto fail_early; \
    } } while (0)

    INIT_MUTEX(flush_lock);
    INIT_MUTEX(q_lock);
    INIT_COND(q_cv);
    INIT_MUTEX(stage2_lock);
    INIT_COND(stage2_cv);
    INIT_MUTEX(commit_exec_lock);

    t->fd = zbd_open(path, O_RDWR, &t->info);
    if (t->fd < 0) { perror("zbd_open"); goto fail_early; }

    if (nvme_get_nsid(t->fd, &t->nsid) != 0) { perror("nvme_get_nsid"); goto fail_fd; }
    ...
    if (!t->zones || !t->zone_wp_bytes || !t->zone_full) { perror("calloc zones"); goto fail_zones; }

    {
        unsigned int nr = t->info.nr_zones;
        if (zbd_report_zones(t->fd, 0, 0, ZBD_RO_ALL, t->zones, &nr) != 0)
        { perror("zbd_report_zones"); goto fail_zones; }
    }
    ...
    { perror("pthread_create txg_commit_main"); goto fail_zones; }
...
fail_fd:
    zbd_close(t->fd);
fail_early:
    /* Best-effort cleanup of initialized mutexes/conds */
...
void cow_close(cow_tree *t)
{
...

    {
        ...
        uint64_t multi_cmds  = atomic_load_explicit(&t->stat_multi_append_commands, memory_order_relaxed);
        uint64_t multi_pages = atomic_load_explicit(&t->stat_multi_append_pages, memory_order_relaxed);

        uint64_t apply_ns_sum     = atomic_load_explicit(&t->stat_apply_latency_ns_sum, memory_order_relaxed);
        uint64_t apply_ns_samples = atomic_load_explicit(&t->stat_apply_latency_ns_samples, memory_order_relaxed);
        uint64_t flush_ns_sum     = atomic_load_explicit(&t->stat_flush_latency_ns_sum, memory_order_relaxed);
        uint64_t flush_ns_samples = atomic_load_explicit(&t->stat_flush_latency_ns_samples, memory_order_relaxed);
        uint64_t append_ns_sum    = atomic_load_explicit(&t->stat_append_latency_ns_sum, memory_order_relaxed);
        uint64_t append_ns_samples = atomic_load_explicit(&t->stat_append_latency_ns_samples, memory_order_relaxed);
        uint64_t sort_ns_sum      = atomic_load_explicit(&t->stat_sort_latency_ns_sum, memory_order_relaxed);
        uint64_t sort_ns_samples  = atomic_load_explicit(&t->stat_sort_latency_ns_samples, memory_order_relaxed);
        uint64_t zone_rots        = atomic_load_explicit(&t->stat_zone_rotations, memory_order_relaxed);
        uint64_t ov_nodes_sum     = atomic_load_explicit(&t->stat_overlay_nodes_sum, memory_order_relaxed);
        uint64_t ov_nodes_max     = atomic_load_explicit(&t->stat_overlay_nodes_max, memory_order_relaxed);
        uint64_t sync_idles       = atomic_load_explicit(&t->stat_sync_idle_waits, memory_order_relaxed);

        double avg_batch   = batches > 0 ? (double)batch_items / batches : 0.0;
        double avg_ov_nodes = batches > 0 ? (double)ov_nodes_sum / batches : 0.0;
        double apply_us    = apply_ns_samples > 0 ? (double)apply_ns_sum / apply_ns_samples / 1000.0 : 0.0;
        double flush_us    = flush_ns_samples > 0 ? (double)flush_ns_sum / flush_ns_samples / 1000.0 : 0.0;
        double append_us   = append_ns_samples > 0 ? (double)append_ns_sum / append_ns_samples / 1000.0 : 0.0;
        double sort_us     = sort_ns_samples > 0 ? (double)sort_ns_sum / sort_ns_samples / 1000.0 : 0.0;
        double pages_per_cmd = multi_cmds > 0 ? (double)multi_pages / multi_cmds : 0.0;

        fprintf(stderr,
                "[cow_fast] cache tl_hit=%lu ram_hit=%lu disk_reads=%lu\n"
                "[cow_fast] batches=%lu avg_batch=%.2f items=%lu\n"
                "[cow_fast] sync_idle_waits=%lu zone_rotations=%lu\n"
                "[cow_fast] overlay_nodes avg=%.2f max=%lu\n"
                "[cow_fast] multi_append_commands=%lu pages=%lu avg_pages_per_cmd=%.1f\n"
                "[cow_fast] latency_us: sort=%.2f apply=%.2f flush=%.2f append(per_cmd)=%.2f\n"
                "[cow_fast] page_appends=%lu\n",
                ...
                (unsigned long)sync_idles,
                (unsigned long)zone_rots,
                avg_ov_nodes,
                (unsigned long)ov_nodes_max,
                (unsigned long)multi_cmds,
                (unsigned long)multi_pages,
                pages_per_cmd,
                sort_us,
                apply_us,
                flush_us,
    ...
    ram_table_destroy(t);
    free(t->zones);
    free((void *)t->zone_wp_bytes);
    free((void *)t->zone_full);
    zbd_close(t->fd);
...
record *cow_find(cow_tree *t, int64_t key)
{
    ...
    while (!p.is_leaf)
    {
        ...
        for (uint32_t i = 0; i < p.num_keys; i++)
        {
            if (key < (int64_t)p.internal[i].key) { child = p.internal[i].child; break; }
        }
        load_page(t, child, &p);
    }
    for (uint32_t i = 0; i < p.num_keys; i++)
...
void cow_set_expected_workers(cow_tree *t, int workers)
{
    (void)t;
    (void)workers;
    /* cow_fast uses a queue-based pipeline; expected_workers is not needed */
}
