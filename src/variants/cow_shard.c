...
cow_tree *cow_open(const char *path)
{
    ...
    /* Initialize queue shards for cow_btrfs2 */
...
void cow_close(cow_tree *t)
{
...

    {
        ...
        fprintf(stderr,
                "[btrfs2] page_cache tl_hit=%lu ram_hit=%lu disk_reads=%lu\n"
                "[btrfs2] q_lock_wait_avg_us insert=%.2f writer=%.2f\n"
                "[btrfs2] avg_batch_size=%.2f batches=%lu items=%lu\n"
            "[btrfs2] queue_depth avg=%.2f max=%lu writer_empty_waits=%lu\n"
            "[btrfs2] overlay_nodes avg=%.2f max=%lu\n"
            "[btrfs2] append_retries=%lu zone_rotations=%lu\n"
            "[btrfs2] sampled_latency_us append=%.2f batch=%.2f\n"
                "[btrfs2] sampled_stage_us sort=%.2f apply=%.2f flush=%.2f\n"
                "[btrfs2] page_appends=%lu\n",
    ...
    /* Cleanup queue shards for cow_btrfs2 */
    for (int i = 0; i < QUEUE_SHARD_COUNT; i++)
    {
        pthread_cond_destroy(&t->queue_shards[i].cv);
        pthread_mutex_destroy(&t->queue_shards[i].lock);
