...
{
    pthread_rwlock_wrlock(&t->ram_table_lock);

    if (!t->ram_slots)
    {
        pthread_rwlock_unlock(&t->ram_table_lock);
        return;
    }
...
{
    pthread_rwlock_rdlock(&t->ram_table_lock);
...
static inline uint32_t key_to_shard(int64_t key)
{
    return (uint32_t)(((uint64_t)key * 11400714819323198485ull) & (WRITER_SHARDS - 1));
}

static void read_tree_snapshot_shard(cow_tree *t, uint32_t shard, pagenum_t *root_pn, uint64_t *seq_no)
{
    for (;;)
    {
        uint64_t s1 = atomic_load_explicit(&t->shard_seq_no[shard], memory_order_acquire);
        if (s1 & 1ULL)
            continue;

        pagenum_t r = atomic_load_explicit(&t->shard_root_pn[shard], memory_order_acquire);
        uint64_t s2 = atomic_load_explicit(&t->shard_seq_no[shard], memory_order_acquire);
        if (s1 == s2 && ((s2 & 1ULL) == 0))
        {
            *root_pn = r;
            *seq_no = s2;
...
{
    uint32_t z = t->data_zone;
    if (z < DATA_ZONE_START)
        z = DATA_ZONE_START;

    while (z < t->info.nr_zones &&
           atomic_load_explicit(&t->zone_full[z], memory_order_acquire))
    {
        z++;
    }

    if (z >= t->info.nr_zones)
    {
        fprintf(stderr, "single-zone-at-a-time: all data zones exhausted\n");
        exit(EXIT_FAILURE);
    }

    t->data_zone = z;
    atomic_store_explicit(&t->current_zone, z, memory_order_release);
    return z;
}
...
static pagenum_t cow_append_page(cow_tree *t, page *p)
{
    ...
    for (;;)
    {
        ...
        if (++retry > t->info.nr_zones)
        {
            ...
            fprintf(stderr, "single-zone-at-a-time: append failed across all data zones\n");
            exit(EXIT_FAILURE);
        }
    }
}
...
static void load_superblock(cow_tree *t)
{
    ...
    for (uint32_t s = 0; s < WRITER_SHARDS; s++)
    {
        atomic_store_explicit(&t->shard_root_pn[s], INVALID_PGN, memory_order_release);
        atomic_store_explicit(&t->shard_seq_no[s], 0, memory_order_release);
    }

    atomic_store_explicit(&t->shard_root_pn[0], t->durable_sb.root_pn, memory_order_release);
    atomic_store_explicit(&t->shard_seq_no[0], t->durable_sb.seq_no * 2, memory_order_release);
}
...
static void *sb_flusher_thread(void *arg)
{
    ...
    while (!atomic_load_explicit(&t->flusher_stop, memory_order_acquire))
    {
        ...
        pagenum_t root;
        uint64_t seq;
        read_tree_snapshot_shard(t, 0, &root, &seq);
...
static void publish_root_single_writer_shard(cow_tree *t, uint32_t shard, pagenum_t new_root)
{
    uint64_t s = atomic_load_explicit(&t->shard_seq_no[shard], memory_order_acquire);
    if (s & 1ULL)
        s++;

    atomic_store_explicit(&t->shard_seq_no[shard], s + 1, memory_order_release);
    atomic_store_explicit(&t->shard_root_pn[shard], new_root, memory_order_release);
    atomic_store_explicit(&t->shard_seq_no[shard], s + 2, memory_order_release);
...
static void *writer_main(void *arg)
{
    ...
    for (;;)
    {
        ...
        pagenum_t root;
        uint64_t seq;
        read_tree_snapshot_shard(t, (uint32_t)shard, &root, &seq);
        (void)seq;
        ...
        publish_root_single_writer_shard(t, (uint32_t)shard, new_root);
...
cow_tree *cow_open(const char *path)
{
    ...
    if (pthread_rwlock_init(&t->ram_table_lock, NULL) != 0)
    {
        perror("pthread_rwlock_init ram_table_lock");
        for (int s = 0; s < WRITER_SHARDS; s++)
        {
    ...
    {
        perror("zbd_open");
        pthread_rwlock_destroy(&t->ram_table_lock);
        for (int s = 0; s < WRITER_SHARDS; s++)
        {
            pthread_cond_destroy(&t->q_cv[s]);
            pthread_mutex_destroy(&t->q_lock[s]);
...
void cow_close(cow_tree *t)
{
...

    {
        ...
        fprintf(stderr,
            "[ram2] page_cache tl_hit=%lu ram_hit=%lu disk_reads=%lu\n"
            "[ram2] q_lock_wait_avg_us insert=%.2f writer=%.2f\n"
            "[ram2] avg_batch_size=%.2f batches=%lu items=%lu\n"
            "[ram2] page_appends=%lu\n",
...
{
    uint32_t shard = key_to_shard(key);
    pagenum_t root;
    uint64_t seq;
    read_tree_snapshot_shard(t, shard, &root, &seq);
    (void)seq;
