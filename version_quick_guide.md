...
## 6) no-use/cow_ram_nobatch

- 한 줄 요약: 배치 writer 스레드를 제거하고, insert 호출 스레드가 즉시 적용/flush/publish까지 수행하는 직렬화 버전.
- 읽기 경로: thread-local read cache + RAM table cache + O_DIRECT fallback (cow_ram 계열과 유사).
- 쓰기 경로: `cow_insert`에서 overlay 적용 후 즉시 flush/publish하며 요청 큐 비동기 처리 경로를 타지 않음.
- 락 방식: `q_lock`을 insert 전체 경로(스냅샷 읽기 -> overlay 적용 -> flush -> publish) 동안 잡아 write-write를 직렬화.
- 추가 락: durable superblock 기록은 `flush_lock`으로 보호되고, 백그라운드 flusher thread가 주기적으로 반영.
- 성격: 구조는 단순하지만 write 동시성은 가장 보수적인 실험 버전.
...
- "가장 단순한 기준 구현"을 보려면: no-use/cow_tree_v1
- "RAM 캐시 + 단일 writer 배치" 기준은: cow_ram
- "내부 노드 캐시 집중"은: cow_ram_internal
- "flush 병렬화"는: cow_ram_async
- "멀티 writer(shard)"는: cow_ram2
- "노배치 직렬 write 경로"는: no-use/cow_ram_nobatch
- "전역 공유 캐시"는: cow_v3
- "전역 캐시 충돌 완화(멀티웨이)"는: cow_v3_multi_cache
